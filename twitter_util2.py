...
def get_client_target_names():
    ...
    #target_name='drdatascientist'
    #target_name='kirkdborn'
    #target_name='hmason'
    #target_name='ApacheSpark' #  29.2k
    #target_name='databricks' # 28.3
    #target_name='azure' # 664.8k 
    #target_name='apachekafka' # 36.1k 
    #target_name='odersky'
    target_name='gvanrossum'
    return client_name,target_name
def setup_database(client_name,target_name):
    db_name=client_name + ".sqlite"
    ...
    """)
    #c.execute("""drop view v_people_to_follow""")
    c.execute("""create view if not exists v_people_to_follow as
    ...
    #c.execute("""drop view v_friends_not_following_me""")
    c.execute("""create view if not exists v_friends_not_following_me as
     select  friend_id from friends
     except
     select follower_id from followers,friends where follower_id = friend_id""")
...
def load_target_tables(twt,db_connection,db_cursor,target_name):
    ...
    while(next_cursor):
        ...
        print ("Get_followers contains " + str(len(l_followers["ids"])) + " follower ids " )
        print("next_cursor contains " + str(next_cursor)) 
        wait_for_reset(twt)
        followers.append(l_followers["ids"])
        next_cursor = l_followers["next_cursor"]
...
def get_list_to_follow(db_cursor):
    db_cursor.execute("""select * from v_people_to_follow order by 1 desc""")
...
def follow(twt,db_connection,db_cursor,target_followers):
    if target_followers:
        ...
        db_cursor.execute("""
        insert or replace into requested_to_follow (request_id) values(""" 
        ...
        try:
            ...
            if  " unable to follow more " in e.msg or "Invalid or expired token"  in  e.msg or "spam and other malicious activity" in e.msg or "Max retries exceeded" :
                raise
        wait_random_time()
...
def get_list_to_unfollow(db_cursor):
    db_cursor.execute("""select * from  v_friends_not_following_me order by random() """)
...
def unfollow_follow(twt,db_connection,db_cursor,target_followers,friends_not_following):
    ...
    while more_to_unfollow:
        for friendnot in friends_not_following:
            ...
            db_cursor.execute("""
            insert or replace into friends_I_unfollowed (friendun_id) values(""" + str(friendnot[0]) 
            + """)""") 
            db_connection.commit()
            try:
